...
local screen = game:GetService("CoreGui"):FindFirstChild("RobloxGui")
...
if UserSettings and LoadLibrary then
	...
	RbxGui,msg = loadfile("http://localhost/asset?id=45284430")()--LoadLibrary("RbxGui")
	...
	local function updatePlayerContextMenu(player,x,y)
		...
		if playerContextMenu == nil then
			...
			playerContextMenu.Parent = screen
		...
		if x and y then
			...
			if x + playerContextMenu.AbsoluteSize.X >= screen.AbsoluteSize.X then
				x = screen.AbsoluteSize.X - playerContextMenu.AbsoluteSize.X
			end
	...
	local checkIfBoardChanged = function()
		...
		if newBoardType ~= currentBoardType or numStats ~= currentStatCount then
			rebuildBoard(screen, newBoardType, numStats)
		end
	end
...
if contextMenu3d then
	...
	function createContextMenu(otherPlayer)
		...
		playerImage.Image = "http://localhost/thumbs/avatar.ashx?userId=" .. tostring(otherPlayer.userId) .. "&x=352&y=352"
